...
class T1Hist(BaseTask):
    ...
    def _init_buffers(self):
        ...
        self.num_priv = self.cfg["env"]["num_priv"]
        self.num_prop = self.cfg["env"]["num_prop"]
        self.obs_hist_buf = deque(maxlen=self.cfg["env"]["hist_len"])
    ...
    def get_obs_info(self):
        """Returns the number of observations, privileged observations, history, and proprioceptive observations"""
        num_obs = self.num_obs  # do nothing
        num_critic_obs = self.num_privileged_obs
        num_hist = self.hist_len
    ...
    def _compute_observations(self):
        ...
        if len(self.obs_hist_buf) == 0:
            self.obs_hist_buf.extend([obs_buf] * self.hist_len)
        else:
            self.obs_hist_buf.append(obs_buf)
        self.obs_buf = torch.cat([obs_buf, privileged_obs_buf, *self.obs_hist_buf], dim=-1)
        self.privileged_obs_buf = torch.cat([obs_buf, privileged_obs_buf], dim=-1)
