...
import learning as locomotion
from learning.algorithms import PPO, ROAPPO
from learning.env import VecEnv
from learning.modules import (
    ActorCritic,
    ActorCriticHistory,
    ActorCriticBetaHistory,
...
class OnPolicyRunner:
    ...
    def __init__(self, env, train_cfg, log_dir=None, device="cpu"):
        ...
        actor_critic: ActorCritic | ActorCriticHistory | ActorCriticBetaHistory = actor_critic_class(
            num_obs, num_critic_obs, self.env.num_actions, **self.policy_cfg
        ).to(self.device)
        alg_class = eval(self.alg_cfg.pop("class_name"))  # PPO
        self.alg: ROAPPO = alg_class(actor_critic, **self.alg_cfg)
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):
        ...
        if self.log_dir is not None and self.writer is None:
            ...
            if self.logger_type == "neptune":
                from learning.utils.neptune_utils import NeptuneSummaryWriter

                self.writer = NeptuneSummaryWriter(log_dir=self.log_dir, flush_secs=10, cfg=self.cfg)
                self.writer.log_config(self.env.cfg, self.cfg, self.alg_cfg, self.policy_cfg)
            elif self.logger_type == "wandb":
                from learning.utils.wandb_utils import WandbSummaryWriter
        ...
        critic_obs = extras.get("critic", obs)
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for i in range(self.num_steps_per_env):
                    ...
                    if "critic" in infos:
                        critic_obs = self.critic_obs_normalizer(infos["critic"])
                    else:
                        critic_obs = obs
